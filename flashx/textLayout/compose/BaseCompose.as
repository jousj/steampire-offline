package flashx.textLayout.compose
{
   ...
   public class BaseCompose
   {
      ...
      private function composeBlockElement(param1:FlowGroupElement, param2:int) : Boolean
      {
         ...
         var _loc15_:ListItemElement = null;
         var _loc5_:int = 0;
         ...
         var _loc6_:FlowLeafElement = this._textFlow.findLeaf(this._startComposePosition - 1);
         if(_loc6_)
         {
            _loc8_ = _loc6_.getParagraph();
            if((Boolean(_loc8_)) && _loc8_ != this._curElement.getParagraph())
            {
               if(_loc8_.paddingBottom != undefined)
               {
                  this._parcelList.addTotalDepth(_loc8_.paddingBottom);
               }
            }
         }
         var _loc7_:Boolean = param2 == this._curElementStart + this._curElementOffset;
         while(_loc5_ < param1.numChildren && (param2 <= this._stopComposePos || !this.parcelList.atLast()))
         {
            ...
            {
               _loc14_ = this._curParcel.applyClear(_loc3_.computedFormat.clearFloats,this._parcelList.totalDepth,_loc3_.computedFormat.direction);
               this._parcelList.addTotalDepth(_loc14_);
            ...
            if(this._blockProgression == BlockProgression.RL)
            {
               ...
               _loc11_ = _loc3_.getEffectivePaddingRight();
               _loc12_ = _loc3_.getEffectivePaddingLeft();
            }
            else
            {
               _loc9_ = _loc3_.getEffectivePaddingLeft();
               _loc10_ = _loc3_.getEffectivePaddingRight();
               _loc11_ = _loc3_.getEffectivePaddingTop();
               _loc12_ = _loc3_.getEffectivePaddingBottom();
            }
            this._parcelList.pushLeftMargin(_loc9_);
            this._parcelList.pushRightMargin(_loc10_);
            if(_loc7_ && _loc11_ > this._verticalSpaceCarried)
            {
               this._parcelList.addTotalDepth(_loc11_ - this._verticalSpaceCarried);
            }
            this._verticalSpaceCarried = Math.max(_loc11_,0);
            _loc13_ = _loc3_ as ParagraphElement;
            if(_loc13_)
            {
               if(!this._atColumnStart && _loc13_.computedFormat.columnBreakBefore == BreakStyle.ALWAYS)
               {
                  this.advanceToNextParcel();
               }
               if(!(this._atColumnStart && this._parcelList.currentParcel != null && this._parcelList.currentParcel.columnIndex == 0) && _loc13_.computedFormat.containerBreakBefore == BreakStyle.ALWAYS)
               {
                  this.advanceToNextContainer();
               }
               if(!this.composeParagraphElement(_loc13_,param2))
               {
                  return false;
               }
               if(!(this._atColumnStart && this._parcelList.currentParcel != null && this._parcelList.currentParcel.columnIndex == 0) && _loc13_.computedFormat.containerBreakAfter == BreakStyle.ALWAYS)
               {
                  this.advanceToNextContainer();
               }
               if(!this._atColumnStart && _loc13_.computedFormat.columnBreakAfter == BreakStyle.ALWAYS)
               {
                  this.advanceToNextParcel();
               }
            }
            ...
            {
               _loc15_ = this._listItemElement;
               this._listItemElement = _loc3_ as ListItemElement;
               _loc4_ = this.composeBlockElement(FlowGroupElement(_loc3_),param2);
               this._listItemElement = _loc15_;
               if(!_loc4_)
               {
                  return false;
               }
            ...
            if(_loc12_ > this._verticalSpaceCarried)
            {
               this._parcelList.addTotalDepth(_loc12_ - this._verticalSpaceCarried);
            }
            this._verticalSpaceCarried = Math.max(_loc12_,0);
            this._parcelList.popLeftMargin(_loc9_);
            this._parcelList.popRightMargin(_loc10_);
      ...
      public function composeTextFlow(param1:TextFlow, param2:int, param3:int) : int
      {
         ...
         {
            if(Boolean(_loc6_) && (_loc6_ as FlowElement).getAbsoluteStart() >= this._startComposePosition)
            {
            ...
            if(_loc10_)
            {
               while(_loc7_ < _loc10_)
               {
                  ...
                  if(_loc11_.floatType != Float.NONE && _loc11_.absolutePosition < this._startComposePosition)
                  {
                     ...
                     _loc13_ = this._blockProgression == BlockProgression.RL ? _loc12_.elementWidthWithMarginsAndPadding() : _loc12_.elementHeightWithMarginsAndPadding();
      ...
      protected function getFirstIndentCharPos(param1:ParagraphElement) : int
      {
         ...
         while(Boolean(_loc3_) && Boolean(_loc3_ is InlineGraphicElement) && InlineGraphicElement(_loc3_).effectiveFloat != Float.NONE)
         {
               ...
               if(this._curElementOffset >= this._curElement.textLength)
               {
                  do
                  {
                     if(this._curParaElement.hasInteractiveChildren())
                     {
                        ...
                        while(Boolean(_loc14_) && _loc14_ != this._curParaElement)
                        {
      ...
      protected function composeInlineGraphicElement(param1:InlineGraphicElement, param2:TextLine) : Boolean
      {
         ...
         {
            _loc6_ = param1;
            while(Boolean(_loc6_) && _loc6_ != this._curParaElement)
            {
         ...
         if(param3)
         {
            ...
            if(this._blockProgression == BlockProgression.TB)
            {
               if(!this._measuring)
               {
                  ...
                  if(Boolean(param2) && TextFlowLine.getNumberLineListStylePosition(param2) == ListStylePosition.OUTSIDE)
                  {
      ...
      private function applyTextAlign(param1:Number) : void
      {
         ...
         if(this._blockProgression == BlockProgression.TB)
         {
            for each(_loc5_ in this._alignLines)
            {
               ...
               if(Boolean(_loc3_) && TextFlowLine.getNumberLineListStylePosition(_loc3_) == ListStylePosition.OUTSIDE)
               {
                  ...
                  _loc12_ += TextFlowLine.getNumberLineInsideLineWidth(_loc3_);
      ...
      protected function advanceToNextContainer() : void
      {
         ...
         var _loc1_:Parcel = this._parcelList.atLast() ? null : this._parcelList.getParcelAt(this._parcelList.currentParcelIndex + 1);
